...
class EnhancedBayesianTEP:
    ...
    def load_data(self):
        ...
        for name, pred in data['predictions'].items():
            if pred['observed']['dv_obs_mm_s'] != 0:
                ...
                dv_unc = sigma_mm_s if sigma_mm_s is not None else dv_unc_mm_s
                if dv_unc is None:
                    self.logger.warning(f"Missing uncertainty for {name}, skipping")
                    continue
                flybys.append({
                    'name': name,
                    ...
                    'beta_eff': pred['tep_predictions'].get('beta_eff', pred['tep_predictions'].get('beta_initial', BETA_BASELINE * 1e-4)),
    ...
    def log_prior(self, params):
        ...
        # BETA_BASELINE from physics.py is 1.0, so actual coupling is 1e-4
    ...
    def log_likelihood(self, params, flybys):
        ...
        for fb in flybys:
            ...
            dv_pred = fb['dv_pred_base'] * (beta / (BETA_BASELINE * 1e-4))
    ...
    def run_mcmc(self, flybys):
        ...
        # log10_beta ≈ -4 (β = BETA_BASELINE * 1e-4 from physics.py)
...
def main():
    ...
    # Log step summary
    if results:
        analysis.logger.log_step_summary(0, "SUCCESS")
        return 0
    else:
        analysis.logger.log_step_summary(0, "FAILED")
        return 1
