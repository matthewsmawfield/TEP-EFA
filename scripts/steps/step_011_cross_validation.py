...
1. Leave-One-Out (LOO): Exclude each of the 4 primary detections, predict
   using the remaining 3. Uses step005 fitted β values directly so that the
...
The previous implementation:
  - Loaded ALL flybys from step004_tep_predictions.json including
...
The corrected implementation:
  - Loads primary detections (excluded=False, β>0) from step005.
...
class CrossValidator:
    ...
    Loads validated primary detections from step005_fitting_results.json
    (which already applies the Holonomic Hybrid disformal coupling and
    S/N > 2 selection). The raw step004 predictions (at beta_ref=1e-4)
    ...
    def load_primary_detections(self):
        """
        Load the 4 primary detections from step005.

        Returns a list of dicts with keys:
          name, beta_fitted, beta_unc, dv_obs, dv_unc, dv_pred_ref,
          beta_ref, altitude_km
        where dv_pred_ref is the step004 raw prediction at beta_ref=1e-4.
        """
        ...
        for name, entry in fit_data['individual_fits'].items():
            ...
            # Step004 raw prediction at beta_ref = 1e-4
    ...
    def _predict_with_beta(self, beta_loo, flyby):
        """
        Scale the step004 raw prediction by the LOO β.
    ...
    def leave_one_out_cv(self, detections):
        ...
        For each held-out flyby:
          ...
          - Enforce β_loo > 0 (it always should be since all individual
            betas are positive).
          - Scale the step004 raw prediction to get dv_pred at β_loo.
        ...
        for i, test in enumerate(detections):
            ...
            if dv_pred is None:
                self.logger.warning(f"Cannot predict {test['name']}: no step004 raw prediction")
                continue
    ...
    def altitude_stratified_cv(self, detections):
        ...
        Uses the step005 fitted betas directly (no re-fitting).
        """
    ...
    def run_validation(self):
        ...
        self.logger.info(
            "Loading primary detections from step005 (S/N>2, excluded=False, β>0). "
        ...
        all_results = {
            'data_source': 'step005_fitting_results.json (primary detections only, S/N>2)',
...
def main():
    ...
    logger = StepLogger("step_019_cross_validation", PROJECT_ROOT)
