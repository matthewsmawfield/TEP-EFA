...
def load_pipeline_data(logger=None):
    ...
    if logger:
        logger.debug("Loading fitting data from step005_fitting_results.json")
    with open(results_dir / 'step005_fitting_results.json') as f:
        fitting_data = json.load(f)
    
    if logger:
        logger.debug("Loading predictions data from step004_tep_predictions.json")
    with open(results_dir / 'step004_tep_predictions.json') as f:
...
def generate_beta_comparison_figure(fitting_data, output_dir):
    ...
    plt.tight_layout()
    output_file = output_dir / 'step007_figure2_beta_comparison.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    plt.close()
    return output_file
...
def generate_ppn_constraint_figure(fitting_data, output_dir):
    ...
    # Calculate |gamma - 1| for each fit using beta_eff with thin-shell screening
    fits = []
    for name, data in fitting_data['individual_fits'].items():
        if data['fit']['beta_fitted']:
            beta_eff = data['fit']['beta_eff']  # Use beta_eff with thin-shell screening
    ...
    # Add weighted mean gamma deviation using beta_eff
    weighted_beta_eff = fitting_data['overall_analysis']['beta_eff_statistics']['weighted_mean']
    weighted_gamma = 8 * weighted_beta_eff**2
    ...
    plt.tight_layout()
    output_file = output_dir / 'step007_figure3_ppn_constraints.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    plt.close()
    return output_file
...
def generate_screening_profile_figure(output_dir):
    """Generate Figure 4: Chameleon screening profile."""
    ...
    ax.plot(altitudes, phi / 1e18, linewidth=2.5, color=BLUE_GREY_COLORS['primary'], label='Chameleon field φ(r)')
    ...
    ax.set_ylabel('Chameleon field φ (× 10¹⁸ GeV)', fontsize=12)
    ax.set_xlim(0, 50000)
    ax.set_yscale('log')
    ax.grid(True, alpha=0.4, which='both')
    ...
    plt.tight_layout()
    output_file = output_dir / 'step007_figure4_screening_profile.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    plt.close()
    return output_file
...
def main():
    ...
    logger = StepLogger("step_007_visualizations", PROJECT_ROOT)
    start_time = time.time()

    logger.header("STEP 007: VISUALIZATION GENERATION")
