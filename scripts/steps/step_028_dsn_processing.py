...
Author: TEP-EFA Analysis Pipeline
Date: 2026-04-18
"""
...
- Project: TEP-EFA Earth Flyby Anomaly Analysis
- Contact: [Your Email]
...
def main():
    ...
    logger = StepLogger("step_028_dsn_processing", PROJECT_ROOT)
    start_time = time.time()
    
    logger.header("STEP 028: DSN RAW DATA PROCESSING WITH MINIMAL ORBIT DETERMINATION")
    ...
    tep_predictions_file = PROJECT_ROOT / 'results' / 'step007_tep_predictions.json'
    ...
    for mission in available_missions:
        ...
        if prediction_key in tep_data['predictions']:
            ...
            datetime_str = datetime_str.replace('A.D. ', '').strip()
            # Parse the datetime flexibly
            try:
                if ' ' in datetime_str:
                    perigee_time = datetime.strptime(datetime_str, '%Y-%b-%d %H:%M:%S.%f')
                else:
                    perigee_time = datetime.strptime(datetime_str, '%Y-%m-%d')
            except ValueError:
                # Try alternate format
                try:
                    perigee_time = datetime.strptime(datetime_str, '%Y-%m-%d')
                except ValueError:
                    logger.warning(f"Could not parse datetime: {datetime_str}, skipping {mission}")
                    continue
    ...
    output_file = results_dir / 'step028_dsn_analysis.json'
