"""
Step 024: Cross-Corpus Parameter Export Manifest
...
def main():
    logger = StepLogger("step_024_cross_corpus_export", PROJECT_ROOT)
    start_time = time.time()
    
    logger.header("STEP 024: TEP CROSS-CORPUS PARAMETER EXPORT")
    
    # Load all relevant parameters from the modular EFA pipeline
    tep_predictions = load_results('step004_tep_predictions.json')
    space_weather = load_results('step022_space_weather.json')
    holonomy = load_results('step027_holonomy_results.json')
