...
class PlasmaModulationModel:
    ...
    def calculate_plasma_effects(self, flyby_data):
        ...
        return {
            ...
            'derivation': f'Solar activity index F10.7 = {solar_activity} estimated from Chapman model fallback; ±50% uncertainty accounts for ionospheric variability and model limitations',
            'source': 'Chapman ionosphere model fallback when IRI data unavailable',
...
def main():
    ...
    results = {
        ...
        'status': 'estimated',
        'calibration_status': 'chapman_fallback_estimate',
        'data_source': 'Chapman ionosphere model fallback when IRI data unavailable',
        'derivation': 'Plasma modulation analysis with Chapman model fallback for missions without IRI data; ±50% uncertainty accounts for ionospheric variability',
        'recommended_action': 'Replace with actual IRI model data where available'
    }
    ...
    for name, pred in data['predictions'].items():
        if pred['observed']['dv_obs_mm_s'] != 0:
            ...
            results[name] = {
                ...
                'derivation': f'Plasma modulation from {plasma_effects.get("iri_data_source", "unknown")}; ±50% uncertainty accounts for ionospheric variability and model limitations',
                'recommended_action': 'Replace with actual IRI model data where available',
                **plasma_effects
            }
