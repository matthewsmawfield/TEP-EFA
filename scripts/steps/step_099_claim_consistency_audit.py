#!/usr/bin/env python3
"""
Step 099: Claim Consistency Audit
================================
...
class ClaimConsistencyAuditor:
    ...
    def _load_pipeline_constants(self) -> Dict[str, Any]:
        ...
        # BETA_INITIAL is defined in step_004 and step_005, or use physics.py default
        step_004 = self.project_root / "scripts/steps/step_004_tep_model.py"
        beta_initial = BETA_BASELINE * 1e-4  # Default from physics.py
        if step_004.exists():
            content = step_004.read_text()
    ...
    def audit_model_comparison_table(self) -> bool:
        ...
        step_016_output = self.project_root / "results/step016_bayesian_model_comparison_results.json"
        
        if not step_016_output.exists():
            self.violations.append("Model comparison output not found. Run step_016.")
            return False
