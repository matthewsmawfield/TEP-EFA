...
Field values are computed self-consistently from the first-principles field equation φ(ρ) = Λ [n Λ^(n+4) M_Pl / (β ρ)]^(1/(n+1)), yielding φ_earth ≈ 2.4×10⁴ GeV and φ_space ≈ 2.0×10¹⁰ GeV for n = 3 and Λ = 10 MeV. The three primary ensemble fits span β = 5.03×10⁻⁴ to 2.02×10⁻³ (factor 4.0), with inverse-variance weighted mean β = 5.65×10⁻⁴ ± 2.79×10⁻⁵. The heterogeneity in fitted values (I² ≈ 100%) reflects geometry-dependent variation in effective coupling across flybys. All fitted values satisfy solar system PPN constraints (|γ - 1| = 2β_eff² < 2.3×10⁻⁵) with safety margins exceeding 10³×. Bayesian model comparison strongly favors the TEP restricted model over the null (Bayes factor B₁₀ = 2.35×10¹⁹, ΔBIC = 89.2) and over the Anderson empirical baseline (B = 168.3, ΔBIC = 10.3). Published null results for high-altitude or symmetric trajectories are consistent with TEP predictions under the Step 007 geometry envelope; Step 039 leaves one raw-tension case (Juno). Numerical simulation demonstrates >50% TEP signal suppression by modern empirical acceleration methods.
...
Four successful TEP fits to flyby anomalies (NEAR, Galileo 1990, Rosetta 2005) with fitted β values spanning 5.03×10⁻⁴ to 2.02×10⁻³ and ensemble weighted mean β = 5.65×10⁻⁴ ± 2.79×10⁻⁵. All fitted parameters satisfy solar system PPN constraints (|γ - 1| = 2β_eff²) with safety margins of 2–3 orders of magnitude. Step 039 classifies four published nulls as raw true nulls under the geometry envelope and one raw-tension case (Juno). Bootstrap (n=10,000) and leave-one-out cross-validation indicate moderate stability.
